...
resource "azurerm_network_interface" "az-jumpbox-vm-nic" {
  ...
  ip_configuration {
    ...
    subnet_id                     = aviatrix_vpc.az-spoke-vnets["az-eu2-jumpbox-vnet"].public_subnets[1].subnet_id
...
resource "azurerm_network_interface" "az-vm-nics" {
  ...
  ip_configuration {
    ...
    subnet_id                     = aviatrix_vpc.az-spoke-vnets["az-eu2-spoke${count.index+1}-vnet"].private_subnets[0].subnet_id
    private_ip_address_allocation = "Dynamic"
  }
}
